...
files.forEach(file => {
    ...
    if (content.includes(originalFooterP) && !content.includes('1240/7 Kazipara Mirpur')) {
        ...
        console.log(`Could not find footer in ${file} or already added.`);
    }
    ...
    if (content.includes(dictEndMarker) && !content.includes('1240/7 Kazipara Mirpur, 10, Dhaka, Bangladesh')) {
        content = content.replace(dictEndMarker, newDictEntry);
        console.log(`Added translation to ${file}`);
    } else {
        console.log(`Could not find dict in ${file} or already added.`);
    }
