...
files.forEach(file => {
  ...
  // 3. Add the Facebook link ONLY inside the footer copyright <p> tag
  ...
  if (!content.includes('https://www.facebook.com/share/1CiLQmp3z2/')) {
    content = content.replace(/(<p[^>]*dptgnajs[^>]*>)\s*(©KSFL Agro Ltd\. 2026\. All rights reserved\.)\s*(<\/p>)/gi, `$1$2${fbLink}$3`);
  }
